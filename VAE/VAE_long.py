...
class VAE(tf.keras.Model):
    def __init__(self, input_dim, timesteps, hidden_0, hidden_1, hidden_2=30):
...
def vae_loss(x, x_recon, mean, logvar, health, reloss_coeff, klloss_coeff, moloss_coeff):
    ...
    reloss = tr.reduce_mean(reloss)
    # KL divergence loss (also per sample)
    klloss = -0.5 * tf.reduce_sum(1 + logvar - tf.square(mean) - tf.exp(logvar + 1e-8), axis=1)  # shape: (batch_size,)
...
@tf.function
def train_step(x, optimizer, reloss_coeff, klloss_coeff, moloss_coeff):
...
def train_loop(train_dataset, val_dataset, epochs, lr, reloss_coeff, klloss_coeff, moloss_coeff, patience=15):
    optimizer = tf.keras.optimizers.Adam(lr)
    val_loss_min = 9e9
    epochs_without_val_loss_improvement = 0

    begin_time = time()
    print(f'Start training for train data shape: {train_dataset.shape}')
    ...
    for epoch in range(epochs):
        ...
        for batch_x in train_dataset:
            loss, reloss, klloss, fealoss = train_step(batch_x, optimizer, reloss_coeff, klloss_coeff, moloss_coeff)
        ...
        for val_x in val_dataset:
            val_loss, reloss, klloss, fealoss = val_step(val_x)
...
if __name__ == "__main__" and try_train_once:
    ...
    for path in train_paths:
        ...
        arr = resample_dataframe(df, total_timesteps_interp)
        arrays.append(arr)
