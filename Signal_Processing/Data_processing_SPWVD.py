...
# Functions for smoothing, downsampling and windowing, and only downsampling defined
def generate_smoothed_df(input_filename, output_folder, output_filename, smoothing_window_size=5):
    ...
    rolling_cols = [col for col in agg_df.columns if col not in ['Time (cycle)']]  # exclude 'Time'
...
def get_downsampled_and_windowed_smoothe_data(csv_filename, output_folder, csv_output_filename, relevant_col_names, downsample_factor, truncation_loc, overlap_window):
    ...
    time_cycles = smoothed_df['Time (cycle)'].values
...
def spwvd_data_processing(directory, input_dir):
    ...
    extract_to_folder=str(input_dir)
    ...
    cycle_smoothing_output_folder = directory / "Output" / "Low_features_cycle_level_smoothed"
    windowing_output_folder = directory / "Output" / "Low_features_windowed_fully_preprocessed"
    os.makedirs(cycle_smoothing_output_folder, exist_ok=True)
    os.makedirs(windowing_output_folder, exist_ok=True)
    ...
    # Columns
    relevant_col_names = ['Time (cycle)','Amplitude_mean','Energy_mean','Counts_sum','Duration_mean','RMS_mean','Rise-Time_mean']
    smoothed_rel_col_names = ['Time (cycle)','Amplitude_mean_smoothed','Energy_mean_smoothed','Counts_sum_smoothed','Duration_mean_smoothed','RMS_mean_smoothed','Rise-Time_mean_smoothed']
    ...
    overlap_window=200
    # Functions applied to all input files and downsampled files saved
    ...
    return windowing_output_folder 